 ...
 */
#define BPS_BENCH_ITEM(_INITIAL, _FUNCTION, _ROUNDS)                           \
  retrys = _ROUNDS;                                                            \
  _INITIAL;                                                                    \
  time_bench_before();                                                         \
  for (int _r = 0; _r < retrys; _r++) {                                        \
    _FUNCTION;                                                                 \
  }                                                                            \
 ...
 * @param[in] DATASIZE             -bit length of data input to hash functions
 * or block-size of block ciphers
 */
