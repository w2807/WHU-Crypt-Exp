...
void test_sm4_correctness() {
  ...
  unsigned char plaintext[SM4_BLOCK_SIZE] = {0x01, 0x23, 0x45, 0x67, 0x89, 0xab,
                                             0xcd, 0xef, 0xfe, 0xdc, 0xba, 0x98,
                                             0x76, 0x54, 0x32, 0x10};

  // Corresponding ciphertext 0x681edf34d206965e86b3e94f536e4246
  unsigned char correctResult[SM4_BLOCK_SIZE] = {
  ...
  if ((memcmp(ciphertext, correctResult, SM4_BLOCK_SIZE) == 0) &&
      (memcmp(plaintext, decrypted, SM4_BLOCK_SIZE) == 0)) {
...
void test_sm4_performance() {
  ...
  BPS_BENCH_ITEM(encInit(key, encSubKeys),
                 sm4_encrypt_block(ciphertext, encSubKeys, ciphertext), ROUNDS);
  BPS_BENCH_FINAL(SM4_BLOCK_BITS);

  decInit(key, decSubKeys);
  sm4_decrypt_block(ciphertext, decSubKeys, decrypted);
  BPS_BENCH_START("SM4 decryption", BENCHS);
  BPS_BENCH_ITEM(decInit(key, decSubKeys),
                 sm4_decrypt_block(decrypted, decSubKeys, decrypted), ROUNDS);
  BPS_BENCH_FINAL(SM4_BLOCK_BITS);
}
